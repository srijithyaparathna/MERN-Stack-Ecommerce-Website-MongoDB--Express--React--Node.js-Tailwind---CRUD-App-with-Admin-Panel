...
const Item = ({ id, name, image, old_price, new_price }) => {
  ...
  //console.log('Item Props:', { id, name, image, old_price, new_price });
  ...
  return (
    <div className='rounded-xl overflow-hidden shadow-lg'>
      <div className='relative flexCenter group overflow-hidden transition-all duration-100'>
        <Link
          to={`/products/${id}`}
          className='h-12 w-12 bg-white rounded-full flexCenter absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 py-2 z-20 scale-0 group-hover:scale-100 transition-all duration-700'
        >
